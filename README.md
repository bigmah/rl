...
**The reward is distance covered: how far Mario is from where the episode started.** Each step pays the change in that distance, so an episode's return is where he ended up. It is not the length of the path he ran — paid for that, a policy learned to hold the stick still and run 23 circles a minute in a patch a thousand units wide, as fast as Mario goes and getting nowhere. And it is not `forwardVel`, which is what the game believes about Mario and can be enormous while he grinds into a wall. Distance covered counts a long jump (about 48 units a frame), a slope he slides down, and whatever exploit turns out to beat both. It is logged as `covered`, with the path length (`distance`, and `score` per frame) and `forward_vel` beside it so the three can be compared.
...
The observation is 42 floats: position, where he is relative to where the episode started, velocity, forward velocity, the speed just achieved, his facing and the camera relative to it, height above the floor, the floor's steepness, headroom, whether he is on a wall, in water or in the air, his action's group and flags, and the clock.
